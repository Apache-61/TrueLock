...
class TrueLockSeeder:
    """
    Seeds TrueLock with one clean control case and four fraud cases:
      ...
      4. Duplicate invoicing / double billing
         (Suministros Duplicados del Norte -> Cliente Ejemplo Dos)
         Signals: two distinct, individually well-formed CFDIs (different
         UUIDs, so the uuid PK never blocks the second one) for the exact
         same provider, receiver, amount, and a one-time monthly-rent
         concept worded identically - re-issued two days apart and each
         paid in full into the *same* provider account from the *same*
         client account. Each invoice/payment pair reconciles perfectly
         on its own; the fraud only shows up when you notice the client
         paid twice for something that should only be billed once.
        ...
        with psycopg2.connect(self.conn_str) as conn:
            with conn.cursor() as cur:
                try:
                    ...
                    conn.commit()
                    print("Seeding completed successfully. Control case and 4 fraud cases loaded.")
    ...
    def _seed_entities(self, cur):
        ...
        # payee), fraud case 3 (three fresh shells + collector), and
        # fraud case 4 (duplicate-invoice provider + client).
        cur.execute("""
    ...
    def _seed_providers(self, cur):
        ...
        # in case a later re-seed tries to downgrade it. DUPX150101DU1
        # is an otherwise unremarkable, EFOS-clean provider - the fraud
        # case 4 signal lives entirely in the invoices, not here.
        cur.execute("""
    ...
    def _seed_accounts(self, cur):
        ...
        # account per case. For fraud case 4, both the client and the
        # provider get a single account each - both duplicate payments
        # flow through the very same pair of accounts.
        cur.execute("""
            INSERT INTO accounts (account_no, entity_id, bank) VALUES
                ...
                ('777788889999000011', 'entity-dup-provider', 'Banco Demo'),
                ('888899990000111122', 'entity-cliente-dup', 'Banco Demo')
    ...
    def _seed_invoices(self, cur):
        ...
        #
        # FRAUD CASE 4 invoices: two different UUIDs (a real CFDI can
        # never repeat one), same provider, same receiver, same amount,
        # and the exact same "one month's rent" concept re-issued two
        # days apart - each invoice is perfectly well-formed on its own,
        # the duplication only shows up comparing the two side by side.
        cur.execute("""
            INSERT INTO invoices (uuid, version, provider_rfc, receiver_rfc, issue_date, amount, subtotal, taxes, currency, payment_method, payment_form, cfdi_type, concept) VALUES
                ...
                ('77777777-7777-4777-8777-777777777777', '4.0', 'DUPX150101DU1', 'CLI010101CLI', '2024-04-05', 52200.00, 45000.00, 7200.00, 'MXN', 'PUE', '03', 'I', 'Renta mensual de equipo de computo, marzo 2024'),
                ('88888888-8888-4888-8888-888888888888', '4.0', 'DUPX150101DU1', 'CLI010101CLI', '2024-04-07', 52200.00, 45000.00, 7200.00, 'MXN', 'PUE', '03', 'I', 'Renta mensual de equipo de computo, marzo 2024')
    ...
    def _seed_transactions_and_payments(self, cur):
        ...
        #
        # FRAUD CASE 4: two individually clean settlements, both moving
        # money through the exact same client -> provider account pair,
        # two days apart, for what is supposedly a one-time monthly rent.
        cur.execute("""
            INSERT INTO transactions (id, from_account, to_account, transaction_date, amount, related_payment_id) VALUES
                ...
                ('transaction-dup-001', '888899990000111122', '777788889999000011', '2024-04-06', 52200.00, NULL),
                ('transaction-dup-002', '888899990000111122', '777788889999000011', '2024-04-08', 52200.00, NULL)
        ...
        cur.execute("""
            INSERT INTO payments (id, related_invoice_uuid, payment_date, amount, payment_form, currency, previous_balance, remaining_balance, transaction_ids) VALUES
                ...
                ('payment-dup-001', '77777777-7777-4777-8777-777777777777', '2024-04-06', 52200.00, '03', 'MXN', 52200.00, 0.00, ARRAY['transaction-dup-001']::TEXT[]),
                ('payment-dup-002', '88888888-8888-4888-8888-888888888888', '2024-04-08', 52200.00, '03', 'MXN', 52200.00, 0.00, ARRAY['transaction-dup-002']::TEXT[])
            ON CONFLICT (id) DO NOTHING;
        """)
        ...
        cur.execute("""
            ...
            UPDATE transactions SET related_payment_id = 'payment-dup-001' WHERE id = 'transaction-dup-001';
            UPDATE transactions SET related_payment_id = 'payment-dup-002' WHERE id = 'transaction-dup-002';
        """)
    ...
    def _run_integrity_guards(self, cur):
        ...
        cur.execute("""
            ...
            BEGIN
                ...
                SELECT count(*) INTO bad_total FROM transactions
                 WHERE id IN ('transaction-dup-001', 'transaction-dup-002');
                IF bad_total <> 2 THEN RAISE EXCEPTION 'expected 2 duplicate-invoice transactions, found %', bad_total; END IF;
            END
            $$;
        """)
